...
class DRLAgent:
    ...
    def DRL_prediction_load_from_file(model_name, environment, cwd, deterministic=True):
        ...
        # Helper for logging trades per step.
        tickers = getattr(environment, "ticker_list", None)

        while not done:
            # Record holdings before the step to detect executed trades.
            prev_stocks = environment.stocks.copy()
            ...
            price_vec = environment.price_ary[environment.day]
            new_stocks = environment.stocks.copy()
            delta_stocks = new_stocks - prev_stocks

            for idx, delta in enumerate(delta_stocks):
                if delta == 0:
                    continue
                side = "buy" if delta > 0 else "sell"
                ticker = tickers[idx] if isinstance(tickers, (list, tuple)) else str(idx)
                trade_price = float(price_vec[idx])
                shares = int(abs(delta))
                trade_value = trade_price * shares
                trade_records.append(
                    {
                        "step": len(episode_total_assets) - 1,
                        "ticker": ticker,
                        "side": side,
                        "shares": shares,
                        "price": trade_price,
                        "notional": trade_value,
                        "day_index": int(environment.day),
                    }
                )

            total_asset = (
                environment.amount
                + (price_vec * environment.stocks).sum()
            )
        ...
        # Persist trade log if any trades occurred.
        if trade_records:
            os.makedirs(config.RESULTS_DIR, exist_ok=True)
            trades_df = pd.DataFrame(trade_records)
            trades_path = os.path.join(
                config.RESULTS_DIR, f"trades_stable_baselines3_{model_name}.csv"
            )
