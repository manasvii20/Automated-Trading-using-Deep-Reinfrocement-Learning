...
from finrl.config_private import (
    ALPACA_API_KEY,
    ALPACA_API_SECRET,
    ALPACA_API_BASE_URL,
)
...
class AlpacaProcessor:
    def __init__(self, client=None):
        ...
        If a client is passed in and it already supports the modern
        `get_stock_bars` method (i.e. is a StockHistoricalDataClient or
        similar), we reuse it. Otherwise we fall back to creating our own
        StockHistoricalDataClient using the API keys.

        This makes the processor robust when a trading REST client
        (alpaca_trade_api.REST) is accidentally passed in, as happens in
        the paper-trading environment.
        """

        # Reuse a compatible data client if provided
        if client is not None and hasattr(client, "get_stock_bars"):
            self.client = client
            return

        # Otherwise, create our own StockHistoricalDataClient
        try:
    ...
    def fetch_latest_data(
        ...
        new_df = []
        for tic in ticker_list:
            tmp_df = pd.DataFrame(index=times, columns=["open", "high", "low", "close", "volume"])
            tic_df = data_df[data_df.tic == tic].set_index("timestamp")

            tmp_df.update(tic_df[["open", "high", "low", "close", "volume"]])
        ...
        # Ensure there are no NaNs or infs in the features used for the state,
        # otherwise the trading agent will receive invalid observations and
        # Stable-Baselines3 will crash with NaNs in the policy.
        ...
        vix_df = self.client.get_stock_bars(vix_req).df

        # Use a scalar turbulence value to avoid ambiguity in comparisons
        if vix_df.empty:
            latest_turb = 0.0
        else:
            latest_turb = float(vix_df["close"].iloc[-1])
