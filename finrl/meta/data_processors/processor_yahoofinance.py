...
class YahooFinanceProcessor:
    ...
    def download_data(
        ...
        for tic in ticker_list:
            # NOTE:
            # - Intraday intervals (e.g. 1m) are limited by Yahoo to small windows per request.
            # - Daily-or-higher intervals should be downloaded in one shot; downloading 1 day at a time
            #   is extremely slow and looks like the process is "running forever".
            if self.time_interval in {"1d", "5d", "1wk", "1mo", "3mo"}:
                temp_df = yf.download(
                    tic,
                    start=start_date,
                    end=end_date,
                ...
                if temp_df is None or temp_df.empty:
                    # Ensure the ticker still exists in downstream processing.
                ...
                # Download in chunks for intraday data (Yahoo limitation).
                # Yahoo allows max ~7 calendar days per 1m request; using 7 days is safe for all intraday intervals.
                chunk = timedelta(days=7)
                current_tic_start_date = start_date
                temp_parts: list[pd.DataFrame] = []
                while current_tic_start_date <= end_date:
                    chunk_end = min(current_tic_start_date + chunk, end_date)
                    part_df = yf.download(
                        tic,
                    ...
                    if part_df is not None and not part_df.empty:
                        temp_parts.append(part_df)
                    current_tic_start_date = chunk_end
                if temp_parts:
                    temp_df = pd.concat(temp_parts)
                else:
                    temp_df = pd.DataFrame(
                        {
                            "Open": [np.nan],
    ...
    def clean_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if self.time_interval == "1d":
            # Use Timestamp index (not strings) so `.loc[tmp_timestamp]` matches existing rows.
            # Keeping strings here while localizing tmp_timestamp creates extra rows and
            # inconsistent lengths across tickers.
        ...
        for tic in tic_list:
            ...
            for i in range(tic_df.shape[0]):  # fill empty DataFrame using original data
                ...
                if self.time_interval == "1d":
                    # Keep daily timestamps naive and aligned to midnight.
                    tmp_timestamp = pd.Timestamp(tmp_timestamp).normalize()
                else:
